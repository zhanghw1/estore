...
<body>
	...
	<div class="header3">
		...
		<div class="h3_right">
			...
			<div class="tsc">
				<a href="toShopCartServlet">去购物车结算</a> <span class="sj_right"></span>
			</div>
		</div>
	</div>
	<!--头部导航-->
				...
				<dl>
					<c:forEach items="${categorylist }" var="category">
						<dd>
							<h1>${category.name }</h1>
							<p>
								<c:forEach items="${category.categories }" var="secondCategory">
				...
				<div class="c3_b1_c_bottom">
					<ul>
						<c:forEach items="${booklist }" var="book">
							<li><a href="toViewBookServlet?id=${book.id }"><img
									src="${book.image }" height="150px" width="100px" /></a> <a
								href="toViewBookServlet?id=${book.id }">${book.name }</a></li>
						</c:forEach>
					</ul>
				</div>
			</div>
				...
				<c:forEach items="${books }" var="book">
					<li>
						<div class="c3_b2_txt">
							...
							<h2>${description }</h2>
							<p>
								<a href="toViewBookServlet?id=${book.id }">更多精彩，点击进入</a>
							</p>
						</div>
					</li>
				</c:forEach>

...
<body>
						...
						<tbody>
							<c:forEach items="${car.orderLines }" var="entry"
								...
								<tr>
									...
									<td><img src="${entry.value.book.image }" height="100px"
										width="80px"> <span>${entry.value.book.name }<br>
									</span></td>
									<td><span>¥${entry.value.book.price }</span></td>
									<td class="index2"><input type="text" name="num"
										value="${entry.value.num }"><br> <span>有货</span></td>
									<td><span>¥${entry.value.cost }</span></td>
									<td>
										<button>保存</button>
										<button>取消</button>
									</td>
 ...
 -->
			...
			<div class="allButtom">
				...
				<a href="user/toConfirmServlet" class="caozuo">去提交</a> <span>已选择<font>${car.sumNum }</font>件商品&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;总价(不含运费)：<font>¥${car.cost }元</font></span>
			</div>
		</div>
	</div>
